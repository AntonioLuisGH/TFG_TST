...
# %% NoSmoothData preprocessing

NoSmoothData = data

# Remove NaN values from original data
index_nan = NoSmoothData[NoSmoothData.isna().any(axis=1)].index
NoSmoothData = NoSmoothData.dropna()
...
#     NoSmoothData[col] = savgol_filter(NoSmoothData[col], 11, 2)

# %% Convert the 'Date' column to datetime
NoSmoothData['Date'] = pd.to_datetime(df['Date'].drop(index_nan))

# Calculate the time intervals between consecutive dates
intervals = NoSmoothData['Date'].diff()
...
# Set the Date column as the index
NoSmoothData.set_index('Date', inplace=True)

# Resample the data
resampled_data = NoSmoothData.resample(resampling_frequency).mean().interpolate()

# %% Plotting Original vs. NoSmoothData Data
...
for var, color in zip(data.columns, colors):
    ...
    # Plot Original Data
    axes[0].plot(NoSmoothData.index[-5000:-4000], NoSmoothData[var][-5000:-4000],
    ...
    # Plot Resampled Data
    axes[1].plot(data.index[-5000:-4000], data[var][-5000:-4000],
                 color=color, label="With Smoothing", alpha=0.7)
    axes[1].set_title(f'{var} With Smoothing', fontsize=14)
    ...
    # Save the plot to a PDF file
    plt.savefig(f'Smoothing_{var}.pdf', format='pdf')
    plt.show()
