...
def load_my_own_dataset():
    # %%
    # Obtiene la ruta absoluta del directorio actual
    ruta_actual = os.path.dirname(os.path.abspath(__file__))
    # Nombre del archivo CSV
    nombre_archivo = 'raw_data.csv'
    ...
    datos_sin = datos['var_diam'] - \
        datos['var_diam'].rolling(window=ventana).mean()
    ...
    data_test = df[['luminosidad', 'temperatura', 'humedad_rel',
                    'temp_suelo', 'electrocond', 'var_diam_sua_py']].iloc[:-48]
    data_train = df[['luminosidad', 'temperatura', 'humedad_rel',
                     'temp_suelo', 'electrocond', 'var_diam_sua_py']].iloc[:-96]
